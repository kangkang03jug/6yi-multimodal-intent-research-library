...
For `full_text` or `official_html` records, the agent must also review Introduction/Motivation for Research Questions. If no explicit RQ label exists but the authors' objective or core question can be reliably summarized, record an `inferred` question with an Introduction/Motivation locator; do not mechanically leave the array empty. An empty array is valid only when no explicit or reliably inferable question can be identified, or when the reading basis is insufficient. Inferred questions are clearly labeled and never rewritten as original RQ numbers.
